...
public class GenServiceImpl implements IGenService {
	...
	public void generatorCode(TableInfo table, List<ColumnInfo> columns, ZipOutputStream zip) {
		// 表名转换成Java属性名
		String[] className = GenUtils.tableToJavaAndPackage(table.getTableName());
		table.setClassName(className[1]);
		table.setClassname(StringUtils.uncapitalize(className[1]));
		// 列信息
		...
		if (StringUtils.isNotEmpty(className[0])) {
			packageName += "." + className[0];
			table.setPackageContainClassname(true);
