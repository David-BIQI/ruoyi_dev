...
public class GenUtils {
	...
	public static VelocityContext getVelocityContext(TableInfo table) {
		...
		velocityContext.put("package", StringUtils.isNotEmpty(table.getPackageName()) ? table.getPackageName()
				: packageName + "." + table.getClassname());
	...
	/**
	 * 如果表名为: sys_address_city, 则返回数组 下标0: 包名, 下标1: java实体名称
	 * 
	 * @param tableName
	 * @return
	 */
	public static String[] tableToJavaAndPackage(String tableName) {
		if (Constants.AUTO_REOMVE_PRE.equals(GenConfig.getAutoRemovePre())) {
			tableName = tableName.substring(tableName.indexOf("_") + 1);
		}
		if (StringUtils.isNotEmpty(GenConfig.getTablePrefix())) {
			...
			if (indexOf > 0) {
				...
				tables[1] = StringUtils.convertToCamelCase(tables[1]);
				return tables;
			}
			tableName = tableName.replace(GenConfig.getTablePrefix(), "");
		}
		return new String[] { "", StringUtils.convertToCamelCase(tableName) };
	}
	...
	public static String getFileName(String template, TableInfo table, String moduleName) {
		...
		if (!table.isPackageContainClassname() && StringUtils.isNotEmpty(classname)) {
